...
public class Chunk : MonoBehaviour
{
    ...
    public ChunkBlock[,,] voxels; //That bool stores a tridimensional shape, it contains 3 dimensions (represents a cube)  

    ChunkMesher mesher;

    void Awake()
    {
        voxels = new ChunkBlock[size, size, size]; //Apply the size above in the bool (x, y and z)
        ...
        for (int x = 0; x < size; x++) //Render the cubes in X 
            for (int y = 0; y < size; y++)//Render the cubes in Y 
                for (int z = 0; z < size; z++) //Render the cubes in Z
                {
                    ...
                    if (y == groundHeight - 1)
                        type = ChunkBlockType.Grass; // Top block
                    else if (y < groundHeight - 1)
                        type = ChunkBlockType.Dirt; // Underground
                    else
                        type = ChunkBlockType.Air; // Above ground

                    voxels[x, y, z] = new ChunkBlock { type = type };
                }
